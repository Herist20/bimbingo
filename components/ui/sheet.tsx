...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-[var(--bg-base)] p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-300 data-[state=closed]:animate-out data-[state=open]:animate-in',
  {
    variants: {
      side: {
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--brand)] focus:ring-offset-2">
        <X className="h-4 w-4" />
